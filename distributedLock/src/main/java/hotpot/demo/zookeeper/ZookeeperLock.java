...
public class ZookeeperLock implements Lock {
    ...
    @Override
    public void lock() {
        // 使用CountDownLatch来保证当获取到锁后整个方法才退出，比使用轮询更优雅
        ...
        zk.create(LOCK_PATH, "test data".getBytes(),ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL_SEQUENTIAL, (result, path, o, currentNodeFullName) -> {
            if (result == KeeperException.Code.OK.intValue()) {
                ...
                try {
                    List<String> brotherNodes = zk.getChildren(LOCK_NAME, false);
                    tryLock(currentNodeFullName, brotherNodes, lockLatch);
     ...
     * @param children            lock的子节点的名称,如:0000000,0000001,0000003
     */
    ...
    /**
     * 判断当前节点是否是最小的
     * @param currentNodeFullName 当前节点
     * @param brotherNodes 当前节点的兄弟节点
     */
    private boolean isMinNode(String currentNodeFullName, List<String> brotherNodes) {
        Integer min = brotherNodes.stream()
                .map(Integer::valueOf)
